...
class MLP(object):
	...
	def _hidden_layers(self, inputs, training):
		...
		# are allowed to use.
		nn = tf.layers.dense(inputs, self.num_hid_units, activation=tf.nn.relu)
		for i in range(self.num_hid_layers-1):
			nn = tf.layers.dense(nn, self.num_hid_units, activation=tf.nn.relu)
		outputs = nn
		### END CODE HERE
	...
	def _output_layer(self, inputs, training):
		...
		# Note: for tensorflow APIs, only those in tf.layers and tf.nn
		# are allowed to use.
		outputs = tf.layers.dense(inputs, self.num_classes, activation=tf.nn.softmax)
		### END CODE HERE
